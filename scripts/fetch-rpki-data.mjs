...
const pagesSnapshot = new URL('../.cache/snapshots/pages-data.json', import.meta.url);
const pagesSnapshotTemp = new URL('../.cache/snapshots/pages-data.json.tmp', import.meta.url);
await writeFile(pagesSnapshotTemp, JSON.stringify({ dashboard: output, profiles: { generatedAt: output.provenance.generatedAt, watchlist: asWatchlist, byAsn: asProfiles } }));
await rename(pagesSnapshotTemp, pagesSnapshot);
