...
function neighbourTypeLabel(type, language) {
  if (language !== 'zh') return type;
  return { left: '左侧', right: '右侧', uncertain: '不确定' }[type] || type;
}
...
export default function AsProfile({ language }) {
  ...
  return <>
    ...
    {!organizationLoading && queryMode === 'organization' && organizationProfile && <section className="as-organization-detail">
      ...
      <div className="as-metric-strip"><div><span>AS Rank</span><strong>#{formatNumber(organizationProfile.rank || 0)}</strong></div><div><span>{language === 'zh' ? '成员 ASN' : 'Member ASNs'}</span><strong>{formatNumber(organizationProfile.members?.numberAsns || 0)}</strong></div><div><span>{language === 'zh' ? '已观测成员' : 'Seen members'}</span><strong>{formatNumber(organizationProfile.members?.numberAsnsSeen || 0)}</strong></div><div><span>{language === 'zh' ? '客户锥 AS 数' : 'Customer-cone ASes'}</span><strong>{formatNumber(organizationProfile.cone?.numberAsns || 0)}</strong></div><div><span>{language === 'zh' ? '客户锥前缀数' : 'Cone prefixes'}</span><strong>{formatNumber(organizationProfile.cone?.numberPrefixes || 0)}</strong></div></div>
    ...
    {!loading && queryMode === 'asn' && profile && <div className="as-profile-content">
      ...
      {profile.asRank && <section className="as-rank-summary">
        ...
        <div className="as-rank-metrics"><span><small>{language === 'zh' ? '客户锥 AS 数' : 'Customer-cone ASes'}</small><b>{formatNumber(profile.asRank.cone?.numberAsns || 0)}</b></span><span><small>{language === 'zh' ? '客户锥前缀数' : 'Cone prefixes'}</small><b>{formatNumber(profile.asRank.cone?.numberPrefixes || 0)}</b></span><span><small>{language === 'zh' ? '下游 Customer' : 'Customers'}</small><b>{formatNumber(profile.asRank.asnDegree?.customer || 0)}</b></span><span><small>{language === 'zh' ? '对等 Peer' : 'Peers'}</small><b>{formatNumber(profile.asRank.asnDegree?.peer || 0)}</b></span><span><small>{language === 'zh' ? '上游 Provider' : 'Providers'}</small><b>{formatNumber(profile.asRank.asnDegree?.provider || 0)}</b></span></div>
      ...
      <div className="as-two-column">
        ...
        <article className="chart-card as-rpki-card">
          ...
          <div className="as-rpki-chart"><ResponsiveContainer width="100%" height="100%"><LineChart data={chartData} margin={{ top: 10, right: 12, left: -4, bottom: 0 }}><CartesianGrid vertical={false} stroke="#e8edf3" strokeDasharray="3 3" /><XAxis dataKey="date" axisLine={false} tickLine={false} minTickGap={35} tick={{ fontSize: 14, fill: '#7b8794' }} /><YAxis axisLine={false} tickLine={false} tickFormatter={(value) => compact.format(value)} tick={{ fontSize: 14, fill: '#7b8794' }} /><Tooltip content={<AsTooltip />} /><Legend iconType="circle" iconSize={7} wrapperStyle={{ fontSize: 14, paddingTop: 8 }} /><Line type="monotone" dataKey="ipv4" name="IPv4 VRP" stroke="#2563eb" strokeWidth={2} dot={false} connectNulls /><Line type="monotone" dataKey="ipv6" name="IPv6 VRP" stroke="#0891b2" strokeWidth={2} dot={false} connectNulls /></LineChart></ResponsiveContainer></div>
      ...
      {profile.asRank?.asnLinks && <article className="chart-card as-topology-card">
        <div className="card-header"><div><h3>{language === 'zh' ? 'CAIDA 推断的 AS 关系邻域' : 'CAIDA inferred AS relationship neighbourhood'}</h3><p>{language === 'zh' ? '图例给出完整关系数；固定布局只绘制代表性邻居，与下方 RIS 路径邻居语义不同' : 'Legend totals cover all inferred links; the fixed layout draws a representative subset distinct from RIS path neighbours'}</p></div><span className="snapshot-label">AS Rank · {profile.asRank.date}</span></div>
        <AsRelationshipGraph asn={profile.asn} asName={profile.asRank.asnName} connection={profile.asRank.asnLinks} degrees={profile.asRank.asnDegree} language={language} onSelectAsn={openAsn} />
      </article>}

      <article className="chart-card as-neighbours-card">
        <div className="card-header"><div><h3>{language === 'zh' ? 'RIS 观测到的 AS 路径邻居' : 'AS-path neighbours observed by RIS'}</h3><p>{neighbours ? (language === 'zh' ? `按 AS path 相邻组合的出现次数排序 · 显示前 ${topNeighbours.length} / 共 ${neighbours.neighbours?.length || 0} 条记录` : `Ranked by adjacent AS-path occurrences · ${topNeighbours.length} of ${neighbours.neighbours?.length || 0} records shown`) : (language === 'zh' ? '按需加载 RIS 路径邻接记录；不是商业关系判定' : 'RIS path-adjacency records loaded on demand; not a business-relationship inference')}</p></div>{neighbours ? <span className="snapshot-label">{formatTime(neighbours.query_endtime)}</span> : <button className="as-load-neighbours" onClick={() => loadNeighbours()} disabled={neighboursLoading}><Network size={14} />{neighboursLoading ? (language === 'zh' ? '加载中…' : 'Loading…') : (language === 'zh' ? '加载邻居详情' : 'Load neighbour details')}</button>}</div>
        ...
        {neighbours && <><div className="as-neighbour-summary"><span>{language === 'zh' ? '左侧出现' : 'Left'} <b>{neighbours.neighbour_counts?.left || 0}</b></span><span>{language === 'zh' ? '右侧出现' : 'Right'} <b>{neighbours.neighbour_counts?.right || 0}</b></span><span>{language === 'zh' ? '唯一邻居' : 'Unique'} <b>{neighbours.neighbour_counts?.unique || 0}</b></span><span>{language === 'zh' ? '方向不确定' : 'Uncertain'} <b>{neighbours.neighbour_counts?.uncertain || 0}</b></span></div><div className="as-neighbour-table table-scroll"><div className="as-neighbour-row head"><span>ASN</span><span>{language === 'zh' ? '路径位置' : 'Position'}</span><span>{language === 'zh' ? '路径出现次数' : 'Path occurrences'}</span><span>{language === 'zh' ? 'IPv4 / IPv6 peer 观测数' : 'IPv4 / IPv6 peer observations'}</span></div>{topNeighbours.map((item, index) => <div className="as-neighbour-row" key={`${item.asn}-${item.type}-${index}`}><span><b>AS{item.asn}</b></span><span><em className={`as-neighbour-type ${item.type}`}>{neighbourTypeLabel(item.type, language)}</em></span><span>{formatNumber(item.path_count ?? item.power ?? 0)}</span><span>{formatNumber(item.v4_peers || 0)} / {formatNumber(item.v6_peers || 0)}</span></div>)}</div></>}
        <div className="as-card-note"><Info size={13} />{language === 'zh' ? '“左侧/右侧”只表示邻居位于目标 ASN 在 RIS 所见 AS path 的哪一侧；它不表示流量方向、上下游或商业关系。“方向不确定”通常来自目标 ASN 与 RIS collector 直接相邻造成的观测歧义。' : 'Left/right only identifies which side of the target ASN a neighbour occupies in RIS-observed AS paths; it is not traffic direction or a business relationship. Uncertain often reflects direct collector adjacency.'}</div>
      </article>
    </div>}
  </>;
}
